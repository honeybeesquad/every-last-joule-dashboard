...
async function run({ probe = true } = {}): Promise<{ wind: RegionData; solar: RegionData }> {
  try {
    ...
    const note = `Typical-shape fallback: ${(err as Error).message}; Guangxi mixed wind+solar curtailment ~0.5 TWh/yr; NEA 2024 provincial RE monitoring bulletin.`;
    return {
      wind:  buildTypicalWindRegion("guangxi-wind",  15, 0.2, note + " — wind share (~0.2 TWh/yr, coastal/mountain wind, CSG grid)", "2024"),
      solar: buildTypicalSolarRegion("guangxi-solar", 4, 0.3, note + " — solar share (~0.3 TWh/yr, growing PV in Nanning/Guilin corridor)", "2024"),
    };
  }
}
