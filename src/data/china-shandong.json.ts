...
async function run({ probe = true } = {}): Promise<{ wind: RegionData; solar: RegionData }> {
  try {
    ...
    return {
      wind:  buildTypicalWindRegion("china-shandong-wind",  15, 2.5, note + " — wind share (~2.5 TWh/yr, coastal wind corridor + offshore)", "2024"),
      solar: buildTypicalSolarRegion("china-shandong-solar", 4, 4.5, note + " — solar share (~4.5 TWh/yr, 96.3% PV utilisation)", "2024"),
    };
  }
}
