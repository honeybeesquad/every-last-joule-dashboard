...
async function run({ probe = true } = {}): Promise<RegionData> {
  try {
    ...
    const note = `Typical-shape fallback: ${(err as Error).message}; Fujian coastal mixed wind+solar curtailment ~0.6 TWh/yr; NEA 2024 provincial RE monitoring bulletin.`;
