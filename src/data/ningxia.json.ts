...
async function run({ probe = true } = {}): Promise<RegionData> {
  try {
    ...
    return buildTypicalMixedRegion(
      ...
      1.0,
      { wind: 0.5, solar: 0.5 },
      `Typical-shape fallback: Ember GER 2025 / NEA quarterly statistics unavailable as hourly feed (${(err as Error).message}); Ningxia wind+solar curtailment anchored at ~1.0 TWh/yr.`,
      "2024",
      4.5,
      15,
    );
