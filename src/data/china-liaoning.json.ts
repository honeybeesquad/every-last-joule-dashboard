...
async function run({ probe = true } = {}): Promise<RegionData> {
  try {
    ...
    return buildTypicalWindRegion(
      ...
      15,
      1.6,
      `Typical-shape fallback: ${(err as Error).message}; Liaoning wind curtailment ~1.6 TWh/yr; NEA 2024 provincial RE monitoring bulletin (northeast grid transmission constraint).`,
      "2024",
    );
  }
}
