...
const REGION_ID = "india-odisha";
const __dirname = dirname(fileURLToPath(import.meta.url));
const CSV_PATH = join(__dirname, "../../data/historical/india-odisha-gen-daily.csv");
const CSV_SLDC_PATH = join(__dirname, "../../data/historical/india-odisha-sldc-curtailed-daily.csv");
...
async function run(): Promise<RegionData> {
  ...
  const base = buildTypicalWindRegion(
    ...
    9,
    0.1,
    `No CEA CSV present; T3-modelled fallback calibrated to Ember India 2024 (~0.1 TWh/yr wind curtailment, Odisha wind corridor). ` +
...
if (isMain) {
  withFallback(REGION_ID, () => run(), { regionTier: "estimated" })
    ...
    .catch((err) => {
      console.error("india-odisha loader failed", err);
      process.exit(1);
    });
}

export const buildIndiaOdishaData = () => run();
