...
async function run({
  ...
  if (csv) {
    ...
    const windSourceNote = `CENACE Energía Generada Tipo Técnico CSV relay (${csv.rows.length}-hour window, latest: ${csv.latestDate}). Wind (eólica) × ${(WIND_RATE * 100).toFixed(0)}% calibrated-proxy curtailment rate (SENER PRODESEN 2024–2038 + CRE confiabilidad). T2 anchored ±20%.`;
    const solarSourceNote = `CENACE Energía Generada Tipo Técnico CSV relay (${csv.rows.length}-hour window, latest: ${csv.latestDate}). Solar (fotovoltaica) × ${(SOLAR_RATE * 100).toFixed(0)}% calibrated-proxy curtailment rate (SENER PRODESEN 2024–2038 + CRE confiabilidad). T2 anchored ±20%.`;
