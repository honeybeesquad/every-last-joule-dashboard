...
// Task curtailment anchors: solar ~0.08 TWh/yr, wind ~0.02 TWh/yr.
const SOLAR_ANCHOR_TWH = 0.08;
...
const SOURCE_NOTE =
  "T2-anchored: IEMOP RTD archive (iemop.ph) probed — endpoint reachable but does not expose curtailment values (RTD carries SCHED_MW only, no available-capacity column). Calibration rate from IRENA Philippines RE Statistics 2024 (~0.08 TWh/yr solar + ~0.02 TWh/yr wind) applied to typical profiles. No published curtailment rate available from IEMOP/WESM.";
...
export const buildPhilippinesData = () => run({ probe: false });
