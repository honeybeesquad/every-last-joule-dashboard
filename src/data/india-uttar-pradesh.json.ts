...
const REGION_ID = "india-uttar-pradesh";
const __dirname = dirname(fileURLToPath(import.meta.url));
const CSV_PATH = join(__dirname, "../../data/historical/india-uttar-pradesh-gen-daily.csv");
const CSV_SLDC_PATH = join(__dirname, "../../data/historical/india-uttar-pradesh-sldc-curtailed-daily.csv");
...
async function run(): Promise<RegionData> {
  ...
  if (sldc !== null) {
    ...
    const base = buildTypicalSolarRegion(
      ...
      6.5,
      annualizedTWh,
      `UPNLDC (Uttar Pradesh State Load Despatch Centre) direct curtailment — ${sldc.nRows}-day CSV, ` +
  ...
  const base = buildTypicalSolarRegion(
    ...
    6.5,
    0.15,
    `No CEA CSV present; T3-modelled fallback calibrated to Ember India 2024 (~0.15 TWh/yr solar curtailment, Bundelkhand solar corridor). ` +
...
if (isMain) {
  withFallback(REGION_ID, () => run(), { regionTier: "estimated" })
    ...
    .catch((err) => {
      console.error("india-uttar-pradesh loader failed", err);
      process.exit(1);
    });
}

export const buildIndiaUttarPradeshData = () => run();
