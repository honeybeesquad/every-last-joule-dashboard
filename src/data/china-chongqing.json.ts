...
async function run({ probe = true } = {}): Promise<RegionData> {
  try {
    ...
    return buildTypicalMixedRegion(
      ...
      0.22,
      { hydro: 0.6, solar: 0.4 },
      `Typical-shape fallback: ${(err as Error).message}; Chongqing mixed hydro+solar curtailment ~0.22 TWh/yr; NEA 2024 provincial RE monitoring bulletin (Yangtze basin).`,
      "2024",
      5,
      15,
    );
