...
| Atacama | static | Coordinador Eléctrico Nacional / book research | Typical solar shape around UTC 16:30 scaled to 5.9 TWh/year | B2 Playwright attempt reached the public landing page, but the renewable-reduction document path still returned Cloudflare verification |
...
- [Atacama (Chile) uses a typical solar shape after the B2 Playwright attempt](https://github.com/honeybeesquad/every-last-joule-dashboard/blob/v0-build/docs/known-limitations.md#9-atacama-chile-uses-a-typical-solar-shape-after-the-b2-playwright-attempt) - Chile currently uses a solar-shaped 5.9 TWh/year fallback because the public portal is not yet a dependable feed from this environment.
