...
| Atacama | live | Coordinador Eléctrico Nacional direct WordPress XLSX uploads | Monthly solar-reduction workbook parsed to hourly plant-level MWh | Direct solar curtailment from `Resumen-DiarioHorario-Solar`; if the direct XLSX pattern breaks, the loader falls back to the prior typical solar profile |
...
- [Direct-measured coverage remains partial](https://github.com/honeybeesquad/every-last-joule-dashboard/blob/v0-build/docs/known-limitations.md#6-direct-measured-coverage-remains-partial) - Brazil NE and Atacama are direct measured, while several other live renewable regions remain calibrated proxies.
...
- [ERCOT remains on the EIA proxy after the B2 native attempt](https://github.com/honeybeesquad/every-last-joule-dashboard/blob/v0-build/docs/known-limitations.md#8-ercot-remains-on-the-eia-proxy-after-the-b2-native-attempt) - Texas remains a calibrated EIA-based proxy until the native ERCOT dispatch-down path is stable.
- [Atacama (Chile) direct XLSX is monthly and pattern-based](https://github.com/honeybeesquad/every-last-joule-dashboard/blob/v0-build/docs/known-limitations.md#9-atacama-chile-direct-xlsx-is-monthly-and-pattern-based) - Chile now uses direct Coordinador XLSX uploads, but discovery still relies on predictable WordPress upload paths because listing pages are Cloudflare-gated.
