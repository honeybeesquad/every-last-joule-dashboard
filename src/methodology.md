...
This dashboard is a **wasted-energy database**: it estimates the fraction of current Bitcoin network electricity consumption that is already matched by renewable-energy curtailment, with associated-gas flaring tracked separately as a continuous base-load — measured or estimated across 174 regions. The renewable-curtailment figure is a **lower bound on visible waste**, not an upper bound on available waste. All figures are calibrated against publicly reported 2024 curtailment from the relevant grid operator or regulator. Data, sources, assumptions, and known limitations are documented below.
...
**T1a-live-tso (67 regions, ±15% peakGW envelope or ±2σ from 5-year backfill).** Live TSO/ISO/operator hourly feed *and* a calibration rate published by the same jurisdiction's TSO or regulator. Two loader mechanics fall here: *direct-measurement* loaders sum published dispatch-down / constrained-off / spill volumes (Brazil's eleven ONS state clusters — five named Northeast states plus six South/Centre-South states, with a catch-all bucket for unmapped rows; the per-fuel splits for Belgium, Denmark, France, the UK North Sea, and Alberta — see §2.2; New Zealand, the five AEMO states); *calibrated-proxy* loaders take published hourly generation and multiply by a region-specific 2024-anchored rate (CAISO at 4.25% per fuel, ERCOT West/East at 6.15% wind + 4% solar, WA-SWIS at 8% via AEMO WEM Facility SCADA, Japan at 10% via Kyushu Electric area-demand CSV, most ENTSO-E bidding zones). Both mechanics start from a live grid feed and an own-jurisdiction anchor; they differ only in whether the operator publishes curtailment directly or whether we infer it from generation. The diurnal shape in both cases is real, not assumed. Default envelope is ±15% of peakGW; replaced with 2σ of annual peakGW from the 5-year backfill where the historical archive supports it.

**T1b-live-domestic-anchored (4 regions, ±50% peakGW envelope).** Live feed plus a calibration rate sourced from a *domestic statistical agency* or modelled share-split of a national anchor — i.e., the rate's scope and the live feed's scope do not coincide. The four current zones are Italy-Sardinia and Italy-North-Zone (TERNA bidding-zone live feed; rate from a national anchor split by zone-share), Netherlands (TenneT live feed; rate modelled from the CBS national renewables share), and the Baltics (live feeds for Estonia/Latvia/Lithuania; rate from a joint Baltic anchor). The ±50% envelope is empirical — the post-B1 rerun on 2026-04-26 measured a P67 fractional residual of 0.50 across these four zones, reflecting systematic anchor-scope offset that 2σ on the live series cannot capture.
...
**T2-annual-calibrated (2 regions, ±20% peakGW envelope).** A static flat-base profile anchored to a published annual total, with no diurnal modelling. Currently Austria APG and Russia Murmansk wind. The four flare regions (Permian, W. Siberia, S. Iraq, E. Saudi) carry the same envelope but appear as a separate "flare" bucket on Figure 4 because their flat 24/7 base-load shape is the *physical truth* of upstream gas flaring rather than a modelling concession.

**T3-modelled (96 regions, ±40% peakGW envelope).** A typical-shape profile (solar / wind / hydro-seasonal / mixed / overnight) scaled to a published annual anchor. Used where the annual total is confidently reported but no hourly upstream exists. The eight Chinese provincial regions are the largest T3 block, calibrated against NEA 2024 utilisation rates and public provincial generation data (65.4 TWh/yr, ~77% of NEA-implied national 2024 renewable curtailment/spill — the [China provincial methodology](https://github.com/honeybeesquad/every-last-joule-dashboard/blob/main/docs/methodology/china-provinces.md) carries the full audit). T3 also covers most of South Asia, Africa (the 26 sub-Saharan grids added in the 2026-04-27 Pattern-D bulk audit), the Middle East outside flare, Latin America outside Brazil/Atacama (16 small Caribbean and Central American grids added in the same Pattern-D batch), and Hawaii. Kenya's geothermal venting uses a specialised overnight-concentrated profile (§2.3).
...
**Per-fuel region split (Pattern-PF, 2026-04-27).** Six high-volume blended loaders — CAISO, Belgium, Denmark, France, the UK North Sea, and Alberta — were restructured to emit **two distinct region records per loader**: one solar (`<region>-solar`) and one wind (`<region>-wind`), at separate centroids matching each fuel's geographic concentration (e.g., `caiso-solar` over the Mojave; `caiso-wind` over Tehachapi/Altamont). Each record carries a hard-set `fuelShare` of `{ solar: 1, wind: 0 }` or `{ solar: 0, wind: 1 }` and its own per-fuel calibration rate. This corrects a structural bug in the prior single-record-with-`fuelShare` treatment: at midnight, a blended record with `fuelShare = { solar: 0.4, wind: 0.6 }` would attribute 40% of the (overnight, wind-dominated) curtailment to "solar" — producing a non-zero solar series after sundown that did not reflect physical reality. With the per-fuel split, the dashboard's solar series goes correctly to zero overnight wherever solar generation does, and the wind series is decoupled from solar's diurnal envelope. The split is a structural change at the loader layer; downstream rendering and aggregation logic are unchanged.
...
- **T1 sub-tier subdivision (CODEX-7).** The T1 live-feed tier was subdivided into T1a-live-tso (63 regions at the time, now 67, own-jurisdiction rate, ±15% / 2σ), T1b-live-domestic-anchored (4 regions, domestic stat-agency or modelled-split rate, ±50% empirical), and T1c-live-neighbour-anchored (1 region, neighbour-extrapolated rate, ±35.5% empirical) per the empirical anchor-coverage analysis at `scripts/calibration/empirical_tier_bands.py --by-derivation` and the post-B1 rerun on 2026-04-26. The locked envelopes (±50% for T1b, ±35.5% for T1c) replace the original ±20–25% / ±30–40% provisional bands because the rerun's per-zone residual against published anchors was wider than the pre-rerun analytic estimate. Pre-2026-04-25 snapshots retain the legacy `T1-live-TSO` label as an alias of T1a for envelope sizing.

- **Per-fuel region split (Pattern-PF, 2026-04-27).** The single-record-with-`fuelShare` treatment for blended-feed regions had a structural bug: at hours when one fuel's generation goes to zero (most obviously, solar overnight), the per-fuel split of the *blended* curtailment volume produced a non-zero "solar" series after sundown — because the `fuelShare` ratio was applied to the still-non-zero wind component. Six high-volume loaders were restructured to emit two distinct records per loader, one per fuel, at separate centroids and with hard-set `fuelShare` values: CAISO (`caiso-solar` Mojave / `caiso-wind` Tehachapi-Altamont), Belgium, Denmark (replacing the earlier denmark-west/east geographic split), France, the UK North Sea (replacing the earlier gb-scotland/gb-england-wales geographic split), and Alberta. The change adds 4 new T1a-live regions net (170 → 174 total; T1a 63 → 67) and is detailed in the per-fuel-split methodology block in §2.2.
