...
function interpolateHour(profile: number[] | null | undefined, utcHour: number): number {
...
export function regionGWAtHour(data: RegionData, utcHour: number, mode: DashboardMode = "avg30d"): number {
  ...
  const profile = mode === "last24h" && Array.isArray(data.latestProfile)
    ? data.latestProfile
    : data.profile;
  return interpolateHour(profile, utcHour);
}

/** Gross renewable generation at a UTC hour, when the loader emitted it. */
export function generationGWAtHour(data: RegionData, utcHour: number): number {
  if (!data) return 0;
  return interpolateHour(data.generationProfile, utcHour);
}
