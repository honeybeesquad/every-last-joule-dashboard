# World Coverage Audit Subagent — Asia-Central + Middle-East

## Mission

You are auditing the world's electricity grid operators against the Every-Last-Joule project's tier model. Your scope is **Asia-Central + Middle-East**. You will produce one CSV row per grid operator / bidding zone in your scope and write the result to `data/coverage-audit/2026-04-26-asia-central-middle-east.csv`.
...
## Your scope: All grid operators across Asia-Central + Middle-East. Cross-reference existing project regions in src/lib/regions.ts. Document operators that are reachable AND those that are not — both produce valid rows.
...
```bash
python3 scripts/validation/lint_coverage_audit.py data/coverage-audit/2026-04-26-asia-central-middle-east.csv
```
