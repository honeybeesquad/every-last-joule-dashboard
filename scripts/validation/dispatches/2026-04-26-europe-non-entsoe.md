# World Coverage Audit Subagent — Europe-non-ENTSO-E

## Mission

You are auditing the world's electricity grid operators against the Every-Last-Joule project's tier model. Your scope is **Europe-non-ENTSO-E**. You will produce one CSV row per grid operator / bidding zone in your scope and write the result to `data/coverage-audit/2026-04-26-europe-non-entsoe.csv`.
...
## Your scope: Non-ENTSO-E European operators including UK post-Brexit (note: covered in Task 7 for continuity, list as `leave-existing-T1+` here with cross-ref note), Switzerland (covered in Task 7 as observer), plus:
...
```bash
python3 scripts/validation/lint_coverage_audit.py data/coverage-audit/2026-04-26-europe-non-entsoe.csv
```
