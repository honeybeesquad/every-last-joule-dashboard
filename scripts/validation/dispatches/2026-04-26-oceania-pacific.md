# World Coverage Audit Subagent — Oceania-Pacific

## Mission

You are auditing the world's electricity grid operators against the Every-Last-Joule project's tier model. Your scope is **Oceania-Pacific**. You will produce one CSV row per grid operator / bidding zone in your scope and write the result to `data/coverage-audit/2026-04-26-oceania-pacific.csv`.
...
## Your scope: All grid operators across Oceania-Pacific. Cross-reference existing project regions in src/lib/regions.ts. Document operators that are reachable AND those that are not — both produce valid rows.
...
```bash
python3 scripts/validation/lint_coverage_audit.py data/coverage-audit/2026-04-26-oceania-pacific.csv
```
