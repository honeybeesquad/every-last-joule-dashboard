...
// Produced by scripts/research/brazil-ons-calendar-year.mjs — ONS's own
// frustrated-generation definition (GNRa). The earlier 2026-05-06 artifact used
// val_geracaoreferenciafinal, a REL-only settlement column, and is superseded.
const BRAZIL_STAMP = "2026-09-10";
const BRAZIL_ANNUAL_CSV = path.join(
  RESEARCH_DIR,
  `${BRAZIL_STAMP}-brazil-ons-calendar-year-${YEAR}.csv`,
);
const BRAZIL_FORMULA =
  "sum(max(val_geracaoreferencia - val_geracao, 0) * 0.5h) over half-hours where val_geracaolimitada is non-null (ONS GNRa definition; val_geracaonaorealizadaapurada read directly where the file carries it)";
...
function brazilRows() {
  ...
  return sourceRows
    // brazil-other-* is the residual bucket for unmapped state codes, not a region.
    ...
    .map((row) => {
      ...
      return {
        ...
        source_field_formula: BRAZIL_FORMULA,
        interval_hours: 0.5,
        ...
        research_artifact: `docs/research/${BRAZIL_STAMP}-brazil-ons-calendar-year-${YEAR}.md`,
        notes: `ONS GNRa definition per the dataset dictionary. Reason split: ENE ${fmt(num(row.ene_twh), 3)} / CNF ${fmt(num(row.cnf_twh), 3)} / REL ${fmt(num(row.rel_twh), 3)} TWh. val_geracaoreferenciafinal (REL-only settlement column) and the val_geracaolimitada cap are not used as energy.`,
      };
    })
...
function main() {
  ...
  md.push("");
  md.push("Formula: ONS's own frustrated-generation definition (`val_geracaonaorealizadaapurada`, GNRa, per the dataset dictionary) — `sum(max(val_geracaoreferencia - val_geracao, 0) * 0.5h)` over half-hours where `val_geracaolimitada` is non-null. Files from 2026 carry GNRa as a column and it is read directly; the recomputation reproduces it to the MW.");
  md.push("");
  md.push("Not used: `val_geracaoreferenciafinal` (computed by ONS only for REL half-hours, for CCEE settlement — a fraction of curtailment) and the integral of `val_geracaolimitada` (the cap ONS imposed, not lost energy). `brazil-other-*` residual buckets are excluded because they are not regions.");
