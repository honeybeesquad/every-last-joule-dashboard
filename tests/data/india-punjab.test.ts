...
import { buildIndiaPunjabData } from "../../src/data/india-punjab.json";

describe("india-punjab loader", () => {
  it("returns a valid positive RegionData shape with T3-modelled tier", async () => {
    const data = await buildIndiaPunjabData();
    expect(data.regionId).toBe("india-punjab");
  ...
  it("solar profile peaks around UTC 06-07 (Punjab ~75°E local noon)", async () => {
    const data = await buildIndiaPunjabData();
