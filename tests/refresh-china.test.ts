...
const CSV = [
  ...
  // Hainan has no published curtailment rate in the (honest) table, so it is
  // skipped by buildAnchors — only Xinjiang (transcribed rates) appears.
  "China,CHN,Hainan,HI,2024-01-01,Wind,False,0.12",
  "China,CHN,Hainan,HI,2024-02-01,Wind,False,0.11",
].join("\n");
...
describe("refresh-china buildAnchors", () => {
  ...
  it("skips provinces with no published curtailment rate", () => {
    // CSV has Xinjiang (transcribed rate) + Hainan (no published rate). Only
    // Xinjiang appears; Hainan is skipped by the rate===undefined -> continue path.
    const anchors = buildAnchors(CSV);
