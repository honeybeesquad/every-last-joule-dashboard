...
describe("uncertainty.deriveTier", () => {
  it("maps live → T1-live-TSO", () => {
    expect(deriveTier({ regionTier: "live" })).toBe("T1-live-TSO");
  });
...
describe("uncertainty.computeBounds", () => {
  it("T1 without observed std falls back to ±15%", () => {
  ...
  it("T1 with observed std uses 2σ", () => {
  ...
  it("lower bound clamped to 0 when 2σ exceeds peak for T1", () => {
    // Realistic scenario: noisy small-signal region where backfill σ > peak/2
    const { uncertaintyLowGW, uncertaintyHighGW } = computeBounds(0.5, "T1-live-TSO", 1.0);
  ...
  it("upper bound is always ≥ peak even on degenerate input", () => {
    const { uncertaintyHighGW } = computeBounds(1, "T1-live-TSO", 0);
