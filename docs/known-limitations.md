...
### 11. Flare regions were flat because flare is flat — historical, the bucket is now empty

**This limitation no longer applies to the dataset.** Associated-gas flaring was purged in [#242](https://github.com/honeybeesquad/every-last-joule-dashboard/pull/242) (2026-06-18, renewables-only pivot): all 37 flaring regions and the `T2-flare` presentational bucket were removed. No region in `src/lib/regions.ts` carries a flare tier or a flare kind today, and `deriveTier` has no flare branch — the `Region.tier === "flare"` value it once keyed on is not a member of `RegionTier`. The entry is kept, and kept at this number, because pre-purge snapshots and the v1.0.0–v1.3.2 releases in `dataset/CHANGELOG.md` still contain flare rows, and a reader of those needs the reasoning below.

What it said, and why it was right at the time: Permian, W. Siberia, S. Iraq, and E. Saudi sat in the dataset's flare bucket — a presentational subdivision of `T2-annual-calibrated` shown separately on Figure 4 (the coverage map) so readers could visually distinguish flat 24/7 base-load heat from dispatch-down curtailment. The envelope model was the same ±20% of `peakGW`, and `confidenceTier` returned the single label `T2-annual-calibrated` for both groups; the split was purely a label for the figure. Flare's flatness was methodologically correct: upstream oil production doesn't stop overnight, so the heat output is 24/7 base load. That flat shape was the truth, not an absence of hourly data — which is why the flat profile was not treated as a data gap.

Two live consequences remain. `scripts/validation/figure4_coverage_map.py` still recognises the legacy `flare` / `static` spellings so it can render pre-purge snapshots, and the committed `docs/figures/figure4_coverage_map.{png,pdf}` plus the paper's Technical Validation §4.5 still show the flare bucket, because both are pinned to the 384-region v1.3.1 vintage (see §4.5's own note). Neither is a claim about the current dataset.
