...
The historical-backfill archive (`data/historical/curtailment_backfill.parquet`, published as a Zenodo asset) contains hourly reconstructed curtailment observations for the period **2020-01-01 to 2026-04-24** (2,590,195 rows × 7 columns, ≈ 20 MB Snappy-compressed) for the 29 dataset regions whose upstream source exposes a multi-year archive. It is distinct from the rolling `curtailment_history.parquet` which samples the *live dashboard's current state* on each build.
