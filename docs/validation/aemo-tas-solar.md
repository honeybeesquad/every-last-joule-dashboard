...
Solar units are selected from the local AEMO DUID fuel map. The child row is single-fuel and does not carry `fuelShare`.

## Links

- Loader source: [`aemo.json.ts`](../../src/data/aemo.json.ts)
- Backfill archive: `data/historical/backfill/*_aemo-tas-solar_*.parquet` (0 years)
