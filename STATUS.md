...
Branch `feat/dark-by-default`. A first visit now opens in dark (Horizon)
whatever the OS prefers; this replaces the redesign's D1, where a first visit
followed `prefers-color-scheme` and kept following live OS changes.

- **`src/lib/theme-boot.ts`:** nothing stored → `dark`. The
  `prefers-color-scheme` listener is gone. A stored `light`/`dark` still
  wins, the old `sunfire`/`deepcurrent` values still map to dark (D2), and
  `/embed/` still pins `embed`.
- **No-JS / pre-boot fallback in `src/style.css`** is the dark ground only
  (the light `@media` block is gone).
- **`tests/theme-boot.test.ts`** pins the new rule: dark for either OS
  preference and without `matchMedia`, no OS listener, and an unreadable or
  unknown stored value falls back to dark.

## No Google Fonts request (2026-09-24)

Branch `fix/drop-google-fonts-stylesheet`, the follow-up to the `/privacy`
page (**#1105**, merged 2026-09-24). `observablehq.config.ts` now sets
`globalStylesheets: []`. Left unset, Framework adds a Source Serif 4
stylesheet from `fonts.googleapis.com` and a `fonts.gstatic.com` preconnect
to every page, so every visit sent the visitor's IP address to Google.
`/privacy` drops its Google Fonts bullet and the "apart from one font
stylesheet" clause. Its effective date stays 24 September 2026, the day
#1105 shipped; move it if this merges on a later day.

**Verified:** in a local build, no page in `dist/` loads from either host.
The one remaining match is the provenance link in `/fonts/SOURCES`, which a
browser follows only on a click. **No typography change:** Framework's
`body { font: … var(--serif) }` is the only rule that names Source Serif 4,
and `src/style.css` sets body's `font-family` in every mode. A Playwright
sweep of all 545 built pages in light and dark found no element, `::before`
or `::after` whose computed `font-family` names Source Serif, and no request
to any host but the site's own. A control element set to `var(--serif)` was
caught. Gates pass (1,665 tests). **Not changed:** no data, loader,
`regions.ts` entry or tier.

## Privacy page at `/privacy` (2026-09-24)

**#1105**, merged 2026-09-24. Meta's app review for the social-publishing app
needs a privacy-policy URL that doesn't 404. `src/privacy.md` is a plain doc
page in the About page's layout. The site header comes from the config's
`header`, as on every doc page. Linked from About and listed in the sitemap.
Every claim was checked against the code and the live site:

- **Vercel Web Analytics** (the `/_vercel/insights/script.js` loader in
  `observablehq.config.ts`): no cookies; results seen only as totals.
- **Vercel hosting logs.**
- **`localStorage["elj-theme"]`**: the mode switch's choice. It never leaves
  the browser.
- **Email sent to simon@collins.nu.**
- **Google Fonts.** Framework's default theme adds a Source Serif 4
  stylesheet link to every page. The site doesn't use that face, but the
  browser still requests the stylesheet from `fonts.googleapis.com`, so the
  page named Google. The request and the bullet are both gone now: see "No
  Google Fonts request" above.

There is **no newsletter signup or other form** that sends data. The brief
assumed one; the page says there is none.

**Not reviewed by Simon yet.** It's a good-faith product privacy page, not
legal advice. **Not covered:** `/particle` (Every Last Particle, a separate
project proxied onto this domain). **Not changed:** no data, loader,
`regions.ts` entry or tier.

## Dark globe: a plain scroll over it zooms (2026-09-24)

Branch `feat/dark-globe-wheel-zoom`. A mouse wheel or two-finger scroll
now zooms the dark globe when the pointer is over the globe's disc. Over the
sky, the hero or the dock it still scrolls the page, and at the zoom's limit
in the scroll's direction it passes through (`wheelZooms` in
`src/lib/horizon.ts`), so scrolling down past the whole globe carries on
down the page. Ctrl/⌘ + scroll and pinch still zoom anywhere on the canvas.
A mouse notch zooms about 26%.

**Verified:** gates pass (1,665 tests). In a local build at 1440×900, with
scripted wheel events: sky passes through, globe zooms, four notches down
reach the whole globe and the next scrolls the page, Ctrl + scroll zooms
over the sky. (The browser pane's scroll tool sends no wheel events, so no
real-wheel test.) **Not changed:** no data, loader, `regions.ts` entry or
tier.

## Dark timeline: the ribbon takes the dock's spare width (2026-09-24)

Branch `fix/dark-timeline-fluid-width`. On dark desktop (≥1100px) the
ribbon's grid column was capped at 620px with a stretchy spacer after it,
so wide screens showed a growing gap before the controls. The columns are
now `minmax(0, 1fr) 32px auto`: the ribbon stretches or squashes with the
screen and the gap stays 32px. The canvas already redraws on resize. CSS
only; gates pass (1,664 tests). **Not changed:** no data, loader,
`regions.ts` entry or tier.

## Dark globe: opens zoomed out; drags no longer stop it turning (2026-09-24)

Branch `feat/dark-globe-home-zoom`, a follow-up to #1101.

- **The dark globe opens at 4/9× (`HORIZON_HOME_ZOOM`)**, two steps of the −
  button from the horizon: the globe lifted most of the way off the stage
  floor, countries readable, borders faintly on. Reset returns here. This
  replaces the redesign's full-horizon opening view; 1× (two + steps) is
  still the horizon.
- **The globe keeps turning after a drag, in both modes.** A drag used to
  switch follow-the-sun off until "Now"; #1100 made a zoom do the same, so
  almost any touch froze it. Now a drag holds the camera only while the
  pointer is down, and on release it follows the sun again from where it
  was left (`lonShift`, degrees from the sun). A zoom no longer stops it.
  "Now" and reset clear the shift.

**Verified:** `npm run typecheck && npm test && npm run ci:gates` pass
(1,664 tests), no golden file touched. In a local build at 1440×900 the
page opens at the new zoom, and after a drag the globe keeps turning with
the clock from the dragged position.
**Not changed:** no data, loader, `regions.ts` entry or tier.

## Dark globe: zoom out to the whole globe (2026-09-24)

Branch `feat/dark-globe-zoom-out`, a follow-up to #1100. Zooming in on the
edge-on horizon only magnified the foreshortening; the fix is zooming out.

- **Below 1× the horizon lifts into the whole globe.** It shrinks while its
  centre rises from below the stage to a sphere that fits between the header
  and the dock (`--globe-top-clear` / `--globe-label-clear`). On a wide stage
  it sits right of the hero; on a tablet it is centred. The minimum zoom is
  the whole globe's radius over the horizon's (about 0.21 at 1440×900).
- **The camera tilts toward north-up** (latitude 20) as it zooms out,
  added to the visitor's own tilt. Borders fade in toward the whole globe
  (0.22 alpha). Beams and dots shrink more slowly than the globe.
- The − button, Ctrl/⌘ + scroll and a pinch all reach it; the − button is
  now enabled at home. The home view is unchanged.
- Pure and tested: `horizonFullGeometry`, `minZoom`, `zoomOutBlend`, and
  `viewGeometry` / `clampView` / `zoomViewAt` / `borderAlpha` taking the
  minimum zoom.

**Verified:** `npm run typecheck && npm test && npm run ci:gates` pass
(1,663 tests), no golden file touched. Looked at in a local build at
1440×900 (home, halfway, whole globe) and 820×1100 (whole globe).
**Not changed:** no data, loader, `regions.ts` entry or tier.

## Dark globe: zoom, tilt and borders (2026-09-24)

Branch `feat/dark-globe-zoom`. The dark "Horizon" globe had no zoom and a
drag turned longitude only, so countries were hard to make out.

- **Zoom about the pointer:** Ctrl/Cmd + scroll, a trackpad pinch (Safari's
  gesture events too), a touch pinch in the phone explorer, and a glass
  +/−/reset column at the stage's top right (tablet and desktop, dark only).
  1× to 8×. A plain scroll still scrolls the page. Zooming stops
  follow-the-sun as a drag does; "Now" or reset turns it back on.
- **Vertical drag tilts** the camera latitude between -85 and 25, so any
  latitude can come to the horizon (it was fixed at -36).
- **Country borders** (the 110m atlas's mesh) fade in from 1× to 2.5×. At
  home the stage draws exactly as before. Beams grow as √zoom, not zoom,
  so they do not swamp the land; stars stay in the stage's sky.
- The view maths is pure and tested (`viewGeometry`, `clampView`,
  `zoomViewAt`, `borderAlpha` in `src/lib/horizon.ts`).

**Verified:** `npm run typecheck && npm test && npm run ci:gates` pass
(1,657 tests) with no golden file touched. Looked at in a local build at
1440×900 and 820×1100 in dark (zoom, tilt, hold while the clock plays,
reset, plain scroll not captured), and light shows no new control.
**Not changed:** no data, loader, `regions.ts` entry or tier.

## Nightgrid + Daylight: the modes' colours, type and mark (2026-09-24)

Branch `feat/nightgrid-daylight-palettes`, **stacked on PR 5 (#1094), which is
stacked on #1093 and #1092; those three merge first, in that order**. The
redesign's layouts, globes and token names are unchanged; only token values,
faces and the mark's colours move. The palettes were chosen on the "ELJ
colour scheme concepts" design canvas.

- **Dark mode (Horizon layout) takes Nightgrid:** navy-black grounds
  (`#060a11` / `#0b121d` / `#15202f`), cream ink `#fff8e0`, gold brand
  `#ffd05a`, fuels gold `#ffd05a`, cyan `#67e8f9` and violet `#9d8cff`.
- **Light mode (Almanac layout) takes Daylight:** paper `#f6f0e0`, navy ink
  `#0b121d`, muted copy `#484c50`, a deep gold `#8a5a00` for brand text, fuels
  `#b07800`, `#08778d` and `#7a4fd0`, stale ring `#c2410c`.
- **Hydro is violet in both modes.** Sunfire's cyan and periwinkle, the pair
  Nightgrid grew from, were 5 apart (CIELAB) under simulated deuteranopia. A new test in
  `tests/theme-contrast.test.ts` holds every fuel pair at least 30 apart
  under simulated deuteranopia, protanopia and tritanopia, in both modes.
- **Type is the brand system's in both modes:** Schibsted Grotesk (display,
  body, wordmark) and IBM Plex Mono 500 (figures, labels). PR 5 deleted both
  files; they are restored byte-for-byte from git. Geist, Geist Mono and
  Newsreader are deleted. The Newsreader italics are gone: the hero's %
  sign is upright, and "Fig. 1" and the est./stale tags are mono labels.
- **The wordmark is the system lockup:** 700 at -0.01em, "Joule" in
  `--brand-text`, one header size (17px, 18px on desktop) in both modes.
- **The mark is back in the system's colours.** Dark: gold disc, gold pillars
  at 55%, the cyan curtailed arc at 95% (a new `--mark-lead-opacity` token).
  Light: a paper variant, disc `#b07800`, pillars `#e6a020`, arc `#0b8aa3`,
  so the disc (3.3:1) and arc (3.6:1) hold on paper. `npm run brand:assets`
  regenerated `src/brand/*.svg` and the inline marks in the dark palette.
- **The paper figure (`data-theme="embed"`) is unchanged**, on purpose: it
  is the figure as the DARI paper published it.
- `readGlobeTokens` falls back to the Nightgrid values.

**Verified.**
- **Gates:** `npm run typecheck && npm test && npm run ci:gates` all pass
  (1,652 tests), with no golden file touched.
- **Overflow:** all 543 non-embed pages at 320, 375 and 390, both modes:
  3,258 measurements. Every page fits except `/fonts/SOURCES.html`, a page
  Framework builds from the font notes and nothing links to. It overflows
  36px at 320 in both modes, exactly as a build of PR 5 does.
- **Faces:** in a browser, the dashboard loads only Schibsted Grotesk and IBM
  Plex Mono in both modes. The methodology page also loads Inter 400 and
  700, which is second in the font stack.
- **Looked at:** the dashboard at 1440 and 390 and the methodology page at
  1440 and 390, in both modes. There was no pixel diff against PR 5, because
  every colour and face was meant to change.

**Not changed.** No data file, loader, `regions.ts` entry or tier is
touched.

**Follow-ups.**
- The brand design system (claude.ai artifact) still lists Sunfire as its
  only theme. It should gain Nightgrid and Daylight.
- `/fonts/SOURCES.html` is built as a page at all; either exclude it from
  the build or let its table wrap.
...
- **The paper figure's palette is `data-theme="embed"`, no longer
  "sunfire".** The boot script, `src/embed/globe.md` and the token block in
  `src/style.css` all use the new name, with identical tokens. The site's
  modes are light and dark only. The old picker's stored `sunfire` and
  `deepcurrent` values still map to dark (D2), because returning visitors
  may hold them for years.
- **Unused faces are deleted:** IBM Plex Sans and Mono, Schibsted Grotesk
  and the 16 Gotham `.ttf` files, 21 files in all. Every page's loaded faces
  were checked in a browser first: the site loads Geist, Geist Mono and
  Newsreader, and the paper figure loads Fraunces and Inter, which stay.
- **Dead CSS is removed:** nine classes nothing in `src/` produces
  (`.display-xl`, `.display-lg`, `.display`, `.mono`, `.on-dark`, `.dot-brand`,
  `.dot-orange`, `.methodology-toc`, `.region-tooltip-source`), the legacy
  alias tokens that nothing read, and the display type-scale tokens only
  those classes used.
- **`/regions`** no longer says the dashboard lists "the top 50 per fuel". It
  lists the ten largest, across fuels, and the line was already out of date
  before the redesign.

**Verified.**
- **Gates:** `npm run typecheck && npm test && npm run ci:gates` all pass,
  with no golden file touched.
- **The paper figure is unchanged:** against a build of PR 4, 0 pixels
  differ outside the canvas in either mode, and the G1 loop still runs.
- **The site is unchanged:** against a build of PR 4, the dashboard and the
  doc pages differ only within the same-build noise floor, at desktop and
  phone widths in both modes.
- **Overflow:** all 542 pages at 320, 375 and 390, both modes: 3,252
  measurements, all fit.

**Not changed.** No data file, loader, `regions.ts` entry or tier is
touched.

## Light/dark redesign, PR 4: tablet and phone, the explorer, the doc pages' header (2026-09-24)

Branch `feat/redesign-tablet-phone` (**#1093**), **stacked on PR 3 (#1092),
which must merge first**. The plan is
`docs/superpowers/plans/archive/2026-09-23-light-dark-redesign.md`; its "PR 4 as
built" section lists every departure from the spec.

**Phones (≤640px).**
- **Light** (the light phone board): a 64px header with the mode button and
  a menu; the hero with the time after the eyebrow; a static globe that
  bleeds past both edges, with "Explore the globe"; two stats (the network
  hashrate is left out, as the board does); ten rows with their " GW"; then
  the legend, the timeline full width and the controls in wrapping rows.
- **Dark** (the dark phone board): one screen. The horizon is drawn into a
  band under the hero (R = 650, the limb 235px into the band, latitude −38°,
  beam 0.12, width 0.75, dots 0.8), with a glass "Explore the globe" pill.
  The dock is a bottom sheet (`src/components/sheet.js`): 132px collapsed,
  with the two stats and the region that leads; tapping or dragging the
  handle opens it to 70% of the screen (the ribbon, the controls, the top
  ten). It is a named region: focus moves in when it opens, and Escape
  closes it.
- **The explorer** (`src/components/explorer.js`, both modes): the globe's
  own stage, fixed over the page. It takes touch (drag, tap to select,
  pinch in light), has a 44px close button, a play button and a scrubber,
  and shows the selection in a bottom card. It is a modal dialog: focus
  starts on close, stays inside and returns to the trigger. It pushes a
  history entry, so Android back and Escape close it.
- Outside the explorer the phone globe is a picture (no touch handlers, so
  a vertical swipe scrolls) and redraws once per hour change, in both modes.

**Tablet (641–1099px).** Light: hero and globe side by side, the rail
(list and legend) and the timeline full width below. Dark: the Horizon stage
with the dock split into two stacked glass cards.

**Header on every page.** Below 960px the nav and the DOI pill move into a
menu (`src/components/site-chrome.js`: aria-expanded; Escape, a click
outside or focus leaving closes it). The doc pages (Regions, History,
Methodology, Paper, About and every `/region/<id>`) now get the dashboard's
header: `observablehq.config.ts` builds it at build time with
`src/lib/site-header.ts`, the same nav and menu pieces the dashboard uses.
Their DOI pill reads `dataset/CITATION.cff`. Each doc page has a small JS
block that mounts the mode switch and the menu. The pages' own
"← Dashboard" row and content are unchanged.

**Verified.**
- **Gates:** `npm run typecheck && npm test && npm run ci:gates` all pass.
  The run covers 236 files and 1,644 tests, and no golden file is touched.
- **Phones at 390×844** beside both phone boards, and tablets at 768×1024
  and 1024×768, in both modes.
- **Desktop is unchanged:** against a build of PR 3, 0 pixels differ outside
  the canvases at 1440 and 1100 in both modes (1 pixel of noise in one
  run).
- **Interactions** (headless, touch): the sheet opens on a tap and on a
  drag, moves the stats with it, and closes on Escape with focus back on
  the handle. The explorer opens as a modal dialog with focus on close,
  selects by tap (Sichuan in dark, Iceland in light), and closes on Escape
  and on history back, restoring focus and scroll. The menu opens, focuses
  its first link and closes on Escape.
- **Frame budgets** (Chrome with GPU, 390×844, 4× CPU throttle): the page
  16.7ms a frame while playing, in both modes. In the explorer, dark 16.7ms
  playing and dragging; light 33.3ms playing (at the 30 fps budget) and
  16.7ms dragging.
- **Overflow:** all 542 pages at 320, 375 and 390, both modes: 3,252
  measurements, all fit.
- **Embed:** 0 pixels differ from PR 3's build outside the canvas, and the
  G1 loop still runs.
...
`docs/superpowers/plans/archive/2026-09-23-light-dark-redesign.md`; its "PR 3 as
built" section lists every departure from the spec.

**Layout.** At ≥1100px in dark the first viewport is a stage (100svh, at
least 720px). The globe canvas fills it behind the header (no rule), the
hero sits at (48, 128) over a radial scrim, and a glass dock sits 24px in
from the sides and bottom, 196px tall: the timeline (1fr) and the rail
(420px), each glass, joined along the rail's hairline. The footer follows
the stage. It is the same DOM as light, with every ID kept.

**Globe.** Dark draws the horizon (`src/globe-horizon.js`, ported from the
handoff's reference; geometry in `src/lib/horizon.ts`, tested):
- R = 0.94 × max(width, 900), the limb's top at half the stage height, the
  centre at 57% of the width. Latitude is fixed at −36°. Longitude follows
  the sun (+20°) until the visitor drags; a drag turns longitude only, and
  "Now" turns following back on. There is no zoom: the wheel scrolls the
  page.
- The backdrop (sky, stars, atmosphere, body, rim) is cached offscreen. Land
  dots sit every 0.42°. The lit territory is `globe-surface.ts`'s, with the
  horizon's radius (0.5 + 1.6√GW): `assignTerritory` now takes the radius as
  a parameter and reports each dot's strength.
- Beams: height 0.004 + 0.1√GW (in R), width 1.1 + 1.2√GW, additive.
  Brightness is the quality bucket's `qualityOpacity` (1, 0.8, 0.62); an
  estimated beam also gets a dashed core, and a stale feed a dashed
  `--quality-warning` ring at the base. Beams from just beyond the horizon
  rise from the limb.
- A click selects a beam: full brightness, an ink ring at the tip, and a
  glass label card that tracks the tip. A mouse hover opens the detail card,
  now glass in dark.
- It draws on change, never on an idle loop. Light and dark also skip clock
  ticks under 15 s of clock time, so "Now" no longer redraws every frame.
- G1 now runs only for the Sunfire-pinned embed, unchanged.

**Dock, hero and tokens.**
- Timeline: the stacked ribbon (hydro, wind, solar) with an ink playhead
  and a grid in the hairline's hue, under "Global curtailment · 24 h ·
  30-day average" and a glowing fuel key.
- Controls: a 44px brand play button, 1×/4×/8× and Now, the time window and
  the units ("GW", "% gen"), in Mono 10.5 uppercase.
- Rail: "Largest now" with "All N" (N = the regions `/regions` lists), five
  rows with glowing bars, "est." and "stale" tags kept. The legend reads
  "Brighter beam = higher confidence" and keys the dashed core (est.) and
  the dashed ring (stale).
- Hero: a glowing dot and the live time in the eyebrow, the 184px figure,
  the muted lead, and the stats in one row with short Mono labels (the long
  labels stay for screen readers).
- New tokens in both modes: `--globe-star-rgb` and `--bar-track`.
  `readGlobeTokens` adds the sky, stars and beam tips; `cssRGB()` gives
  canvas code a token's hue.

**Interim until PR 4.** Below 1100px dark stacks in one column, like light,
with the horizon drawn in the globe cell.

**Verified.**
- **Gates:** `npm run typecheck && npm test && npm run ci:gates` all pass.
  The run covers 232 files and 1,611 tests, and no golden file is touched.
- **Dark at 1440×900** beside `mocks/dark-desktop.html`, and at 1100×760
  and 1920×1080.
- **Light is unchanged:** against a build of `main`, 0 pixels differ outside
  the canvases at 1440, 1100, 768 and 390 beyond the same-build noise
  floor, and the light globe's canvas matches.
- **Interactions** (headless, dark): hover opens the glass card; a click
  selects "Sichuan · 6.8 GW now · Estimated", and the card tracks the tip
  through a drag; the wheel scrolls the page; switching to light keeps the
  hour and the selection and fires one `themechange`.
- **Frame budgets:** in Chrome with GPU at dpr 2, 16.7ms a frame at
  1440×900 and 1920×1080, playing at 1× and 8× and dragging, with no long
  tasks. Paused and on "Now", the globe draws nothing. The interim phone at
  4× CPU throttle: 16.7ms median.
- **Overflow:** all 542 pages at 320, 375 and 390, both modes: 3,252
  measurements, all fit.
- **Embed:** 0 pixels differ from `main` outside the canvas, in either
  mode; inside it the difference equals the same-build noise floor. The G1
  loop still runs, and the canvas keeps its fixed accessible name.
...
Branch `feat/light-almanac-desktop` (**#1091**, merged 2026-09-23 as
`3b272ec6`). The plan is
`docs/superpowers/plans/archive/2026-09-23-light-dark-redesign.md`; its "PR 2 as
built" section lists every departure from the spec.
...
Dark and the Sunfire-pinned embed kept the G1 renderer; PR 3 (above) gives
dark the horizon.
...
- D4: the speeds were 1×, 4× and 8× plus Now, with the clock defaulting to 1×.
  **Superseded 2026-09-24:** 0.5× is back and is the default again (1× ran
  too fast to read); the light timeline's controls column widened from 224px
  to 272px to fit the fourth chip.

**Interim until PR 3 and PR 4.** Dark used the same grid, with its hero
capped at 132px and bar rows, until PR 3 (above). Below 1100px the parts stack in one column,
...
Branch `feat/light-dark-theme-plumbing` (**#1087**, merged 2026-09-23 as
`9ef3f7ad`). The first of five PRs that build the
...
`docs/superpowers/plans/archive/2026-09-23-light-dark-redesign.md`, the handoff's spec
...
- `/methodology`: 446px wide at 375 (four runs; ten at 320).
- `/paper`: 549px at 375.
- `/history`: 341px at 320 only.
- `/region/<id>`: 482–501px on every region page checked (12 before the fix).
  The provenance panel's Source link is a bare URL. It set the min-content of
  the panel's `1fr` grid track, so the whole panel grew with it. At 768 the
  same link still ran 52px past the panel border on three China regions.
- `/about` and `/regions` already fit.

**Fix.** `.methodology-doc` now sets `overflow-wrap: anywhere`, and its tables
and `pre` blocks reset to `normal`. We used `anywhere` rather than `break-word`
because it also lowers min-content, and the grid track needs that to shrink.
Tables keep scrolling inside their own box (Framework's `main table`
`overflow-x: auto`).

**Verified** on 541 pages: the five doc pages plus all 536 `/region/<id>`
pages, at 320, 375 and 390, in both themes. Each page shows
`scrollWidth` = `clientWidth`, and no element outside a table or chart
scroller extends past `clientWidth`. A layout diff against the unpatched CSS
...
**Contract.** `RegionData.wasteStatus`: `measured` | `measured-zero` | `unpublished`. `profile` stays waste GW. Globe draws a generation ring for TSO-collected / unpublished grids; waste pillars only when waste is measured. Live generation + unpublished waste stays **T3**, never T1a. T3+verified remains illegal.

**Inventory.** `data/coverage-audit/tso-grid-roster.csv` (generated from `src/lib/tso-grid-roster.ts`). Collect statuses: `live-tso` / `relay-tso` / `not-collecting` / `no-public-series` / `conflict-blocked`.
...
**`--font-display` was dead code.** It was declared in both themes — Fraunces
on Sunfire, IBM Plex Sans on Deepcurrent — and read by **nothing**. Every
heading, the hero percentage and all body copy resolved through
`var(--font-sans)` → `--font-body` → **Inter**. So the site was Inter
throughout, and the only Fraunces on the dashboard was the italic "Joule" on
the wordmark. Any earlier note in this file describing the dashboard's display
face as a serif was wrong.

This points `--font-display` **and** `--font-body` at Schibsted Grotesk in both
themes, and wires the `.display-*` / `h1`–`h4` rule to `var(--font-display)`
so the token now means something. `--font-mono` (IBM Plex Mono) is untouched:
labels, tabular figures and the loader's counter keep the instrument voice.

**The italic serif "Joule" is gone**, replaced by upright brand face at 700 —
the lockup as drawn on the design canvas, and what the loading screen shows two
seconds earlier. That accent was a deliberate earlier choice ("carried over
from the Ledger redesign because Simon liked it"); reverting is
`.app-wordmark-accent` alone.

**Two details worth keeping:** the `@font-face` range widened from `400 700` to
`400 900`, because the variable file's axis really does run to 900 (measured:
glyph advances keep growing past 700) and `--fw-black` is 800 — declared at
700 it would have clamped and the hero number would have rendered lighter than
it does today. And no latin-ext file is needed: every one of the 459 region
names is inside latin-1 (`ã é í ø`, four names).

**Deliberately excluded:** `src/embed/globe.md` still sets Fraunces. It is the
figure iframed by the DARI paper, so its look is part of a published artefact;
changing it is a separate call. The Fraunces `@font-face` rules and woff2 files
stay for it.
...
standing off a gold disc, the first 26% of them cyan — running the
"sweep and release" loop: up clockwise from 12 over 2.6s, a 0.4s hold at the
full ring, then down clockwise from 12 over 2.0s, each pillar firing a tick
from its tip as it starts to drop. The last tick fades at 6.4s, so the ring is
empty and still for 0.2s before the 6.6s cycle restarts.

**Where it lives.** `scripts/lib/brand-mark.ts` is the single source of truth
for geometry, timing and colour; `npm run brand:assets` renders four files into
`src/brand/` (`mark-loop`, `mark-still`, `avatar-loop`, `avatar-still`) and they
are **committed, not built on deploy** — the loader has to paint from raw HTML
before any script runs, which is also why the motion is SMIL inside the file
rather than CSS in `style.css`: every pillar shares one cycle but enters and
leaves at its own moment, which needs per-element `keyTimes`. CSS can only
stagger with a delay, which smears the reset. `prefers-reduced-motion: reduce`
swaps the `<img>` to `mark-still.svg` (full ring, no animation) via `content:`;
there is no way to pause SMIL in an `<img>` from the host document.

**Colours are existing tokens, not new ones:** gold is `--brand` (#ffd05a),
cyan is the Sunfire `--fuel-wind` default (#67e8f9). `tests/brand-mark.test.ts`
(11 cases) re-renders from the generator and fails if a committed SVG has
drifted, checks both loader files are referenced from `index.md`/`style.css`,
checks all four are in `config.dynamicPaths` (nothing else references the
avatars, so without that they would not be emitted), and asserts the cycle
invariants — rise finishes before the fall starts, rise and fall run in the
same clockwise order, the last tick evaporates before the loop restarts.

**The pillar heights are a fixed hash, and are decorative.** They are not
region data and the mark is not a chart; it is drawn identically in every
...
recorder, not a social upload.

## v1.4.0 minted — version DOI 10.5281/zenodo.22837934 (2026-09-19)

GitHub release [`v1.4.0`](https://github.com/honeybeesquad/every-last-joule-dashboard/releases/tag/v1.4.0) (squash `d1fdbc2d`, PR #1034) minted Zenodo record [22837934](https://zenodo.org/records/22837934). **Version DOI `10.5281/zenodo.22837934`.** Concept DOI remains `10.5281/zenodo.19835411`. v1.3.2 (`10.5281/zenodo.20570864`) is the previous, flare-inclusive mint — not rewritten.

This follow-up records the minted DOI in `CITATION.cff` (archival identifier first, so the dashboard fallback parser picks it), `dataset/README.md`, `FAIR.md` F1, public papers, methodology, and the DARI HTML close. The dashboard banner reads Zenodo live and should show **v1.4.0** on the next successful fetch.

**Still open:** regenerate `docs/figures/figure{1,4}_*` (flare palette). **No tier moved.**

## Paper + methodology reframe — HEAD, renewables-only (2026-09-18)

Branch `docs/paper-and-methodology-reframe`. The May 2026 paper and Scientific Data drafts still described the **v1.3.x flare-era deposit** (385 regions, T2-flare, WooCharts 197.6 TWh Bitcoin). Live HEAD is **459 regions / 191 countries and territories**, renewables only (flare purged 2026-06-18, #242). This rewrite makes the public methodology, `src/paper.md`, `docs/paper/*`, and `docs/dari/paper.html` match that.

**Numbers used in prose (18 September 2026 production, trailing 30d × 365/30):** live-tier (T1a+T1b+T1c) **146 TWh/yr**; T2 **3**; T3 **234**; all published **383**. Bitcoin **134 TWh/yr** at mempool.space 24h hashrate × **16 J/TH** (not WooCharts 197.6). Live-tier / Bitcoin **109%**. Priced live-tier revenue ~**$7.3B/yr**. Golden tally unchanged: T1a 160, T1b 26, T1c 1, T2 23, T3 249.

**Left pinned at the time:** `dataset/README.md`, `CITATION.cff`, `.zenodo.json` were still at **v1.3.2**. Superseded by the 2026-09-19 v1.4.0 bump above.

**Then still open:** regenerate `docs/figures/figure{1,4}_*`. **No tier moved.** Methodology §2.1 headers kept in the `ci:methodology-counts` format.

## Rajasthan honesty + Colombia CSV currency (2026-09-15)

Two stale-label threads, neither a magnitude rewrite.

**Rajasthan (`india-rajasthan`).** The dashboard still plots CEA daily generation × Ember India 2024 6% (≈6.3 TWh/yr). That number is not replaced: the official RRVPNL `re-curtailment` PDFs for Jan–May 2026 extract to **0.052 TWh** (75 rows, 52 manual-from-scan) — a five-month floor, not a source-verified annual, and **>50×** below the modelled anchor ([#964](https://github.com/honeybeesquad/every-last-joule-dashboard/issues/964)). What changed is the label: `source` / `sourceUrl` / `sourceNote` / `docs/validation/india-rajasthan.md` now point at `gen-re.cea.gov.in` and say the SLDC portal is not the live source. T1a promotion still waits on instruction energy, not this modelled path.

**Colombia.** `data/historical/colombia-vertimientos-daily.csv` ends **2026-09-13** (1,775 rows, last pull **#1009**). [#620](https://github.com/honeybeesquad/every-last-joule-dashboard/issues/620) closed 2026-08-19. The STATUS header had still said the relay producer "needs a human" as if the CSV were dead; that sentence is corrected. This is not a claim that the XM capture lake on abed is fully ported — only that the dashboard CSV is current.
...
Live-site copy now says **459 regions, renewables only**. The old OG card and README still mentioned flared gas and 384/461 regions after the 2026-06-18 purge. At merge, the May 2026 paper still reported **385 regions and flare** — that was the v1.3.x deposit, not a silent retcon. Superseded for the paper/methodology files by the 2026-09-18 reframe above. The embed says curtailment *could cover* hashrate; it no longer says the network *is* powered by it. **No tier moved.**

**Follow-ups (not in #968):** methodology §2.1's stale sub-tier counts were corrected in **#979** and are now gated by `ci:methodology-counts` (**#981**) — see the entry above. The May 2026 paper / `docs/paper/` / `docs/dari/` flare-era drafts are rewritten on `docs/paper-and-methodology-reframe` (2026-09-18 entry).

## Secret rotation — EIA key rotated (SEC-1 closed), ENTSO-E deliberately deferred (2026-09-10)

**SEC-1 is closed.** `EIA_API_KEY` was rotated on 2026-09-10, roughly four months after the audit flagged it. EIA issues **one key per email address** — re-registering an address it already knows returns *"You have already registered"* rather than a new key, and `Forgot My API Key` would have re-mailed the existing one, so the new key was registered against a fresh `+alias`. It landed in three places: a rebuilt `.env.local`, **Vercel Production** (type Secret, updated 2026-09-10, carrying a note that records the registration address and the absence of self-serve revocation), and the repository secret described below.
...
- **SEC-1 rotated 2026-09-10.** `EIA_API_KEY` was still carrying the `DH9J…` prefix when this sweep ran, roughly four months after the leak was found. Rotated on 2026-09-10 and verified live in production across all 34 EIA regions — see the secret-rotation entry at the top of this file.
...
- Dashboard banner: **"Wasted Energy Database · v1.4.0"** (pulled live from Zenodo version metadata; v1.4.0 minted 2026-09-19, version DOI `10.5281/zenodo.22837934`)
