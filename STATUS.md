...
**Coverage — full world:**
- 474 regions across 195 countries (every UN member + Taiwan + Palestine) — **renewables-only** (curtailed solar/wind/hydro); flare gas purged 2026-06-18.
- **Tally golden as of 2026-06-21: T1a=172, T1b=28, T1c=1, T2=23, T3=250 (total 474).** Counted directly from `src/lib/regions.ts`. Locked by `tests/regions.test.ts` + `scripts/ci/golden/tier-counts.json`. Austria promoted from T2 anchored to T1b live-domestic-anchored (+2 T1b, −1 T2). Earlier drift (2026-06-10 golden total 385): EIA-930 9 US BAs +18 T2 (#163); China/India/Chile/Colombia/Ukraine expansion (#203/#206); Germany DE-LU → 4 TSO control areas (#214); Mexico per-fuel split (#216); estonia/south-korea per-fuel splits; GGFR flare per-site split (#220 → 468, since purged).
...
| **SHIPPED 2026-06-21 — Austria promoted from T2 anchored to T1b live-domestic-anchored.** Replaced the single static `austria` (T2 anchored, mixed, 0.5 TWh/yr flat profile) with `austria-wind` + `austria-solar` (T1b live-domestic-anchored, both fed from APG's ENTSO-E A75 domain `10YAT-APG------L`). Follows established pattern from Italy bidding-zone and Germany TSO splits (PR #163). T2 24→23, T1b 26→28, total 474. Tally golden: T1a=172, T1b=28, T1c=1, T2=23, T3=250.

**IN FLIGHT 2026-06-21 — Argentina CAMMESA relay (PR #273).** Adds relay infrastructure fetching Argentina wind generation data from CAMMESA's live API through a NordVPN Argentine exit node, committed as CSV for Vercel builds. All regions stay `estimated`. Tally unchanged.

**IN FLIGHT 2026-06-21 — Bangladesh PGCB live HTML parser (PR #277).** Replaces typical-profile fallback with a live parser reading hourly solar/wind generation from PGCB's public dashboard. Solar column verified against live HTML. All regions stay `estimated`. Tally unchanged.
