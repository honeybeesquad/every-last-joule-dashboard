...
**BUILT + VERIFIED 2026-06-23 (PR pending, branch `fix/subhourly-interval-overcount`) — sub-hourly interval `totalTWh` overcounts.** Two loaders pushed sub-hourly `CurtailmentPoint`s without `intervalHours`, so `totalTWh30d` (1 h/point default) counted each sub-hour sample as a full hour. (1) **Brazil ONS** (`brazil-ne.json.ts`) — half-hourly feed → `totalTWh` **2× too high**; fixed with `intervalHours: 0.5` in `parseOnsCurtailmentCsv`. Snapshot regenerated: 28-region 30-day total **5.17 → 2.50 TWh**. (2) **Malaysia** (`malaysia.json.ts`) — 10-min GSO feed → `totalTWh` **6× too high** on the live path; fixed with `intervalHours: 1/6` via new exported `gsoSolarToCurtailmentPoints` (live regen 0.0179 → 0.0031 TWh = ÷5.86 ≈ 6×; committed snapshot left as the anchor fallback so the diff is code+test only). `profile`/`peakGW` were always correct (hour-bucket averaging). Regression tests added to both loaders' suites. Gates green: typecheck + 985 vitest + `ci:gates` + `validate`. Completeness-checked the rest of the sub-hourly loaders (Ireland/AEMO/WA-SWIS/NZ/France/Peru/Japan all handle interval correctly).
