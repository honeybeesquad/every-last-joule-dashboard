...
**BUILT + VERIFIED 2026-06-23 (PR pending, branch `fix/ontario-per-plant-remove`) — removed 12 broken Ontario per-plant regions.** The `ontario-per-plant` loader (#247, all 12 `ieso-*` at `tier: live`/`verified`) computed `curtailment = max(0, AvailableCapacity − Output)` where IESO's `Available Capacity` is the plant **nameplate** (the loader's own comment admits this) — so the metric was the normal **capacity-factor gap**, not curtailment. K2 Wind read ~473 MW average "curtailment" on a 270 MW nameplate; the 12 plants summed to **2.4 TWh/30d wind ≈ 29 TWh/yr — larger than Ontario's entire ~14 TWh/yr wind *generation*** and ~19× the province's own ~1.5 TWh/yr IESO curtailment anchor. Physically impossible; no real per-plant dispatch-down signal exists in that IESO feed. **Removed** the 12 regions + the loader (`ontario-per-plant.json.ts`) + `ontario-plant-coords.ts` + 12 validation docs + index.md wiring; the province aggregate `ontario-wind`/`ontario-solar` (generation × rate, plausible 0.048/0.001 TWh/30d) **remains** as Ontario's representation. **Tally golden T1a 172 → 160, total 473 → 461.** Gates green: typecheck + 970 vitest + `ci:gates` (tally-golden, docs-drift, magnitude-golden) + `validate`. Note: AEMO per-plant (UIGF−cleared) and Peru per-plant (gen×rate) use sound signals and are untouched.
