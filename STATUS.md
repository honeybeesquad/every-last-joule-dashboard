...
**BUILT + VERIFIED 2026-06-24 (PR pending, branch `fix/aemo-aggregate-dedup`) — AEMO per-plant double-count fix (aggregate-minus-named).** The headline total double-counted AEMO's 10 named per-plant DUIDs: `aemo.json` summed ALL DUIDs per state while `aemo-per-plant.json` re-emitted 10 of them as their own regions, and `aggregateAtHour` sums both. Fix (owner chose aggregate-minus-named over remove/flag): a shared `PER_PLANT_DUIDS` set (AVLSF1, BANGOWF1, COOPGWF1, DARLSF1, LIMOSF11, MACARTH1, RYEPARK1, SAPHWF1, STOCKYD1, WDGPH1) in `aemo-unit-map.ts`; `aemo.json` skips those DUIDs (so state aggregates = "rest of state"); `aemo-per-plant.json` registry-gated to exactly those 10; the 10 `aemo-{state}-{fuel}` `source:` strings relabelled. Total now counts each plant once. **Code-only** (like the ENTSO-E precedent) — committed snapshot + magnitude-baseline untouched (prod recomputes 3-hourly); avoids a noisy 50-zone baseline churn. Tests: new `tests/data/aemo-per-plant-dedup.test.ts` + updated `aemo.test.ts`. typecheck + ci:gates + validate green. **Peru's analogous double-count (~0.1%) is NOT fixable this way** — `peru.json`'s national figure is COES generation by *fuel* (no per-plant breakdown to subtract) and COES is cloud-IP-blocked; left documented.
