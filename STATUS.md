...
- **PR #1 — EIA-930 9 second-tier US BAs (SOCO, PACW, PACE, PSCO, AZPS, SRP, IPCO, TEPC, WACM) → 18 T2 regions.** Reuses `buildEiaIsoRegionPerFuel` (canonical `regions.ts` tier overrides the helper's `live` default via `enrichWithTier`). Region count **385 → 403**, T2 6 → 24. All gates + 869 tests green. Still to come: Italy (+8 zones), Germany DE-LU→4 TSO areas (+6, needs ENTSO-E CTA live-probe), GGFR flare splits (+~14, static anchors from the GGFR `Field Name` XLSX).
