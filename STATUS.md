...
**BUILT + VERIFIED 2026-06-23 (PR pending, branch `fix/entsoe-portugal-spain-rates`) — corrected overstated ENTSO-E curtailment rates (Portugal, Spain).** Research against primary sources found two `entsoe.json.ts` rate constants badly too high (limitation #15 placeholders): **Portugal solar 10% → 0.5%** (~25× too high — REN *Dados Técnicos 2024* reports no curtailment; combined wind+solar ~0.4%/213 GWh in 2023 per Synertics/REN), **Portugal wind 3% → 0.5%**, **Spain wind 11% → 6.7%** and **Spain solar 5.5% → 3.9%** (per-technology market figures, OMIE/Danthine 2024; the old "~6.8 TWh/yr REE" note was unverifiable vs REE's own 288 GWh/1.6% aggregate). Code + `docs/methodology/entsoe-rates.md` updated with sourced notes; definitions stated per CLAUDE.md honesty rule. **Code-only** change — the committed `entsoe.json` fallback snapshot is left untouched (prod live-recomputes every 3h via `data-refresh.yml`, applying the corrected rates); magnitude-golden passes (Spain stays within band, Portugal drops below the 0.01 TWh floor). 9 other placeholder zones (Finland/Sweden/Romania/Hungary/Bulgaria/Baltics/Italy-zones) confirmed to have *no* citable curtailed-energy total — left as honest placeholders, with a quarantine list of false figures recorded to avoid. typecheck + ci:gates green.
