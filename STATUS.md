...
**BUILT + VERIFIED 2026-06-23 (PR pending, branch `fix/honesty-source-strings`) — corrected self-contradictory / drift-prone provenance strings.** Honesty-pass fixes from the rate-loader audit, no magnitude changes: (1) **Mexico** `sourceNote` stated "~1.2% of renewable generation curtailed" then "calibrated rate (solar 7%, wind 5%)" — the "~1.2%" was a corruption of the header's "~1.2 TWh/yr"; reworded to "~1.2 TWh/yr total VRE curtailment (… ~6% blended)" so the figure and the rates agree (snapshot regenerated, magnitudes stable at solar 0.8 / wind 0.4). (2) **Hokkaido wind** `source`/`sourceNote` claimed "5.8 GWh/30d" but the measured snapshot is 3.45 GWh and Japanese curtailment is seasonal — dropped the drift-prone absolute figure, kept the stabler "~16% of measured curtailment, minority fuel" (code-only; prod's live feed re-stamps the note on next build). typecheck + ci:gates + 26 targeted tests green.
