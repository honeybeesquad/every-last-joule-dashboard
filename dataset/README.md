...
A versioned, reproducible synthesis dataset of hourly renewable-electricity curtailment, covering 374 regions across 6 continents. Built to support the Bitcoin-curtailment-matching hypothesis (the "Every Last Joule" thesis) but published as a general-purpose open resource.
...
- **`data/historical/figure2_validation_scatter.csv`** — 23 region-year anchor pairs with published TSO / ISO / IMM / SoM / IRENA / Ember annual curtailment totals vs. our backfill reconstruction. Machine-readable evidence table for Figure 2.
...
- **`docs/validation/<region>.md`** — per-region triangulation documents (plus a directory README and a `_template.md` scaffold) against published TSO / ISO / IMM / SoM / IRENA / Ember annual reports, with commit-grade diagnostic prose per region.
...
> Collins, S. (2026). Every Last Joule: an hourly synthesis of renewable-electricity curtailment across 374 regions. _Scientific Data_ (in review). Dataset DOI: [10.5281/zenodo.19835411](https://doi.org/10.5281/zenodo.19835411).
...
- The 374 regions break down by confidence tier as **160 T1-live-TSO** (149 T1a own-jurisdiction, 10 T1b domestic-anchored, 1 T1c neighbour-anchored), **6 T2-annual-calibrated** (flat-base statics on a published annual), and **208 T3-modelled** (typical diurnal/seasonal/mixed/overnight shape scaled to a published annual anchor). Every region carries `confidenceTier` so consumers can filter by precision; see [`../docs/methodology/uncertainty.md`](../docs/methodology/uncertainty.md) and [`../docs/known-limitations.md`](../docs/known-limitations.md).
